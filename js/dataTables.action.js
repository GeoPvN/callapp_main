...
function GetDataTableproduct(tname, aJaxURL, action, count, data, hidden, length, sorting, sortMeth, total) {
    if (empty(data))
        data = "";
    ...
    //Defoult Length
    var dLength = [[-1, 30, 50, 100], ["ყველა", 30, 50, 15]];
...
function GetDataTableReload(tname, aJaxURL, action, count, data, hidden, length, sorting, sortMeth, total) {
    if (empty(data))
        data = "";
    ...
    oTable = $("#" + tname).dataTable({
        ...
        "bJQueryUI": true, 																				//Add jQuery ThemeRoller
        "bStateSave": true, 																			//state saving
        "sDom": "<'dataTable_buttons'T><'H'lft><'dataTable_content't><'F'ip>",
		"oTableTools": imex,
        "sPaginationType": "full_numbers",
        "bProcessing": true,
        "aaSorting": [[sorting, sortMeth]],
        ...
        "fnFooterCallback": function ( nRow, aaData, iStart, iEnd, aiDisplay ) {
        	if(!empty(total)){
	            ...
	            for ( var k = 0 ; k < total.length ; k++ )
	            {
	            	nCells[total[k]].innerHTML = (Math.round(iPage[k] * 100) / 100).toFixed(2) + ' <br />' + (Math.round(iTotal[k] * 100) / 100).toFixed(2) + ' <br>';
	            }
        	}
		},
        "fnServerData": function (sSource, aoData, fnCallback, oSettings) {
...
function GetDataTableServerNosave(tname, aJaxURL, action, count, data, hidden, length, sorting, sortMeth, total) {
    if (empty(data))
        data = "";
...
function GetDataTable_st(tname, aJaxURL, action, count, data, hidden, length, sorting, sortMeth, total) {
    if (empty(data))
        data = "";
  ...
  "aButtons": [ "copy",
    ...
    oTable = $("#" + tname).dataTable({
        ...
        "bJQueryUI": true,                     //Add jQuery ThemeRoller
        "bStateSave": true,                    //state saving
        "sDom": "<'dataTable_buttons'T><'H'l><'dataTable_content't><'F'ip>",
        "oTableTools": imex,
        "sPaginationType": "full_numbers",
        "bProcessing": true,
        "aaSorting": [[sorting, sortMeth]],
...
function SetEvents(add, dis, check, tname, fname, aJaxURL, c_data) {
    ...
    $("#" + dis).on("click", function () {
    	...
    	}else if(dis =='delete_button'){
    		var data = $(".check:checked").map(function () { //Get Checked checkbox array
	            return this.value;
	        }).get();
    	}else if(dis =='delete_button_ar'){
    		var data = $(".check_arch:checked").map(function () { //Get Checked checkbox array
	            return this.value;
	        }).get();
    	}else{
    		var data = $(".check_gas:checked").map(function () { //Get Checked checkbox array
                return this.value;
            }).get();
    	}
    	if(dis == 'delete_id'){
                ...
                success: function (data) {
                    if (data.error != "") {
                        ...
                        if(dis == 'delete_but'){
                        	var start 	= $("#search_start").val();
                			var end 	= $("#search_end").val();
                        	LoadTable3(start, end, 'undefined');
                        }
                        }
                    }
                }
...
function MyEvent(add, dis, check, tname, fname, aJaxURL, c_data, c_id, add_id) {
    if (empty(c_data))
        c_data = "";
        $("#"+tname+" tbody").off("dblclick");
        $("#" + add).off("click");
    ...
    $("#" + tname + " tbody").on("dblclick", "tr", function () {
        ...
        if (empty != "dataTables_empty") {
            ...
            $.ajax({
                url: aJaxURL,
                type: "POST",
                data: "act=get_edit_page&"+c_id+"=" + rID + "&" + c_data,
                dataType: "json",
                success: function (data) {
                    if (typeof (data.error) != "undefined") {
                        if (data.error != "") {
    ...
    $("#" + dis).on("click", function () {
        ...
        for (var i = 0; i < data.length; i++) {
            $.ajax({
                url: aJaxURL,
                type: "POST",
                data: "act=disable&"+c_id+"=" + data[i] + "&" + c_data,
                dataType: "json",
                success: function (data) {
                    if (data.error != "") {
                        alert(data.error);
                        ...
                        $("#" + check).attr("checked", false);
                        if(tname == 'example1'){
                        	GetDataTable("example1", aJaxURL, "get_list_detail", 3, "quest_id="+$("#quest_id").val(), 0, "", 1, "desc");
                        }
                        if(tname == 'example2'){
                        	GetDataTable("example2", aJaxURL, "get_list_detail", 3, "quest_id="+$("#quest_id").val(), 0, "", 1, "desc");
                        }
                        if(tname == 'example'){
                        	LoadTable();
                        }
...
function SetEvents1(add, dis, check, tname, fname, aJaxURL, c_data) {
    if (empty(c_data))
        c_data = "";
        $("#"+tname+" tbody").off("dblclick");
        $("#" + add).off("click");
    ...
    $("#" + tname + " tbody").on("dblclick", "tr", function () {
        ...
        if (empty != "dataTables_empty") {
            var rID = $(nTds[0]).text();
            var rnum = $(nTds[2]).text();
            
            $.ajax({
                url: aJaxURL,
                type: "POST",
                data: "act=get_edit_page&id=" + rID + "&num=" + rnum + "&" + c_data,
                dataType: "json",
                success: function (data) {
                    if (typeof (data.error) != "undefined") {
                        if (data.error != "") {
...
function SetEvents_prod(add, dis, check, tname, fname, aJaxURL, c_data) {
    if (empty(c_data))
        c_data = "";
        $("#"+tname+" tbody").off("dblclick");
        $("#" + add).off("click");
    ...
    $("#" + tname + " tbody").on("dblclick", "tr", function () {
        ...
        if (empty != "dataTables_empty") {
            ...
            $.ajax({
                url: aJaxURL,
                type: "POST",
                data: "act=get_add_pagee&prod_id=" + rID + "&change=" + $("#change").val() + "&" + c_data,
                dataType: "json",
                success: function (data) {
                    if (typeof (data.error) != "undefined") {
                        if (data.error != "") {
    ...
    $("#" + dis).on("click", function () {
    	var buttons2 = {
				"save": {
		            ...
		            click: function () {
					    ...
					    var departament = $("#sub1 tr td");
				        for (var i = 0; i < data.length; i++) {
				            $.ajax({
				                url: aJaxURL,
				                type: "POST",
				                data: "act=disable_prod&id=" + data[i] + "&" + c_data,
				                dataType: "json",
				                success: function (data) {
				                    if (data.error != "") {
				                        alert(data.error);
				                        ...
				                        GetDataTableReload("sub1", aJaxURL, "get_my_prod", 10, "id="+$('#id').val() + "&my_dep=" + $(departament[8]).text(), 0, dLength, 0, "desc",total);
				                        GetDataTableReload("sub2", aJaxURL, "get_my_prod_gif", 13, "id="+$('#id').val() + "&my_dep=" + $(departament[8]).text(), 0, dLength, 1, "desc");
				                    }
				                }
				            });
				        }
				        //Reload Table
				        if ($.isFunction(window.LoadTable)) {
				        	GetDataTableReload("sub1", aJaxURL, "get_my_prod", 16, "id="+$('#id').val() + "&my_dep=" + $(departament[8]).text(), 0, "", 0, "desc");
				        }
				        $('#yesnodelete').dialog("close");
		            }    				
		        },"cancel": {
		            text: "არა",
		            id: "no-cc",
        ...
        GetDialog("yesnodelete","300","auto",buttons2);
    });
