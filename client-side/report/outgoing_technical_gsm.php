<head>
	...
	<script type="text/javascript">
		var aJaxURL		= "server-side/report/outgoing_technical_gsm.action.php";		//server side folder url
		var aJaxURL1	= "server-side/report/sales_statistics_outgoing_gsm.action.php";		//server side folder url
		...
		function getData(){
			...
			end_time = $('#end_time').val();
			    ...
			    $.getJSON("server-side/report/sales_statistics_outgoing_gsm.action.php?start="+start_time + "&end=" + end_time + "&agent=" + agent + "&queuet=" + queuet, function(json) {
		...
		function getData7(){
		 ...
		 function drawFirstLevel(){
			   ...
			   start_time = $('#start_time').val();
			   end_time = $('#end_time').val();
			              $.getJSON("server-side/report/outgoing_prod_category_statistics_gsm.action.php?start="+start_time + "&end=" + end_time + "&agent=" + agent + "&queuet=" + queuet, function(json) {
		...
		function go_next(val,par){
			if(val != undefined){
				var tt = $("#myform_List_"+par+"_from option:selected");
				$( tt ).each(function( index ) {
					$("#myform_List_"+par+"_to").append(new Option(this.text, this.value));
		...
		function go_previous(val,par){
			if(val != undefined){
				var tt = $("#myform_List_"+par+"_to option:selected")
				$( tt ).each(function( index ) {
					$("#myform_List_"+par+"_from").append(new Option(this.text, this.value));
		...
		$(document).on("click", "#show_report", function () {
			...
			parame.agent	= '';
			parame.queuet = '';
			paramm		= "server-side/report/outgoing_technical_gsm.action.php";
		...
		$(document).on("click", "#BUSY_dialog", function () {
			LoadDialog2();
		});
		...
		function LoadDialog2(){
			...
			$.ajax({
			    ...
			    data: parame,
		        success: function(data) {		        	
					$("#busy_dialog").html(data.page.answear_dialog);
					var button = {
							"cancel": {
					            text: "დახურვა",
					            id: "cancel-dialog",
					...
					GetDialog("busy_dialog", 500, "auto", button);
					/* Table ID, aJaxURL, Action, Colum Number, Custom Request, Hidden Colum, Menu Array */
					GetDataTable("busy_table", aJaxURL, "busy_dialog_table&start_time="+parame.start_time+"&end_time="+parame.end_time+"&agent="+parame.agent,5, "", 0, "", 1, "desc");
		...
		<div id="tab-0">
					...
					<tbody><tr>
					   <td>ხელმისაწვდომია<br><br>
					    <select size="10" name="List_Agent_available" multiple="multiple" id="myform_List_Agent_from" style="height: 100px;width: 125px;">
							<option value="200">ანა ანდღულაძე</option>
                            <option value="102">ანი იმნაძე</option>
                            <option value="404">თამარ ქავთარია</option>
                            <option value="201">თამარ ჭედია</option>
                            <option value="401">თამარ ჯოლბორდი</option>
                            <option value="405">თეონა ალავიძე</option>
                            <option value="407">მაია ყიფიანი</option>
                            <option value="406">მარიამ მაჩაბელი</option>
                            <option value="409">მარიამ ქვათაძე</option>
                            <option value="150">ნათია ტოგონიძე</option>
                            <option value="400">ნანა კობაიძე</option>
                            <option value="101">ნატალია ნაჭყებია</option>
                            <option value="193">ნატო მღებრიშვილი</option>
                            <option value="150">ნინი ბუაძე</option>
                            <option value="201">ნინი მარეხაშვილი</option>
                            <option value="403">ნინო კანაშვილი</option>
                            <option value="401">თეკლა გადელია</option>
                            <option value="100">ქეთევან გუგუტიშვილი</option>
                            <option value="402">ქეთევან დვალი</option>
                            <option value="408">შორენა ხუბულავა</option>
						</select>
					</td>
					<td align="left">
			...
			<div id="rest" style="margin-top: 200px; width: 100%; float:none;">
                ...
                <tr>
                    ...
                    <th style="background: #FFBE32; color: #FFFFFF">გამოტოვებული</th>
                    <th>ნაპასუხებია</th>
                    <th>უპასუხოა</th>
                    <th>გამოტოვებული</th>
                </tr>
...
<div id="busy_dialog" class="form-dialog" title="გამავალი ზარი">
<!-- aJax -->
</div>
