...
function drawFirstLevel(){
		...
		$("#report tbody").on("click", "tr", function () {
			 ...
			 var nTds = $("td", this);
		            var rID = $(nTds[1]).text();
		            d_url   ="&start="+start+"&end="+end+"&done="+s_i+"&departament="+done[0]+"&type="+done[1]+"&category="+done[2]+"&sub_category=";
	    	    GetDataTable("report_1", aJaxURL, "get_in_page", 4, d_url+rID, 0);
	     		GetDialog("in_form", "1200px", "auto");}
  ...
  <table class="display" id="report_1" >
                    <thead>
                        <tr id="datatable_header">
                            ...
                            <th style="width: 50%;">განყოფილებები</th>
                            <th style="width: 50%;">კატეგორია</th>
                            <th style="width: 50%;">ქვე-კატეგორია</th>
                            <th style="width: 50%;">სახელი</th>
                            <th style="width: 50%;">ტელეფონი</th>
                            <th style="width: 50%;">ზარის დაზუსტება</th>
