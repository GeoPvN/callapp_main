	...
	<script type="text/javascript">
        ...
        function getmiss(){
			...
			var values = $.map(options ,function(option) {
				...
				parame.agent+="'"+option.value.substring(3, 6)+"'";
			});
			
			parame.start_time = $('#start_time').val();
			parame.end_time = $('#end_time').val();
			parame.act = 'miss_checker';
			
				$.ajax({
			        url: paramm,
				    data: parame,
			        success: function(data) {		        	
						$("#technik_miss_info").html(data.page.technik_miss_info);
						$("#answer_call_by_queue_miss").html(data.page.answer_call_by_queue_miss);		
						$("#answer_call_miss").html(data.page.answer_call_miss);		
						$("#unanswer_call_miss").html(data.page.unanswer_call_miss);								
				    }
			    });
		...
		$(document).on("click", "#miss_dialog", function () {
			LoadDialog3();
		});		
		...
		function LoadDialog2(){
			...
			$.ajax({
		        ...
		        success: function(data) {		        	
					...
					/* Table ID, aJaxURL, Action, Colum Number, Custom Request, Hidden Colum, Menu Array */
					GetDataTable("example2", aJaxURL, "undone_dialog_table&start_time="+parame.start_time+"&end_time="+parame.end_time+"&queuet="+parame.queuet+"&agent="+parame.agent,7, "", 0, "", 1, "desc");
		...
		function LoadDialog3(){
			parame 				= new Object();
			paramm		= "server-side/report/technical.action.php";
			parame.start_time 	= $('#start_time').val();
			parame.end_time 	= $('#end_time').val();
			parame.act 			= 'miss_dialog';
			parame.agent	= '';
			parame.queuet = '';
			...
			$.ajax({
			    ...
			    data: parame,
		        success: function(data) {		        	
					$("#add-edit-form-miss").html(data.page.answear_dialog);
					var button = {
							"cancel": {
					            text: "დახურვა",
					            id: "cancel-dialog",
					...
					GetDialog("add-edit-form-miss", 460, "auto", button);
					/* Table ID, aJaxURL, Action, Colum Number, Custom Request, Hidden Colum, Menu Array */
					GetDataTable("example3", aJaxURL, "miss_dialog_table&start_time="+parame.start_time+"&end_time="+parame.end_time+"&queuet="+parame.queuet+"&agent="+parame.agent,7, "", 0, "", 1, "desc");
...
<div id="tabs" style="width: 99%; margin: 0 auto; height:800px; margin-top: 50px;">
		<ul>
			...
			<li><a href="#tab-4">გამოტოვებული ზარები</a></li>
		</ul>
		<div id="tab-0">
					...
					<tr>
					   	<td>
						    ...
						    <select name="List_Queue_available" multiple="multiple" id="myform_List_Queue_from" size="10" style="height: 100px;width: 125px;" >
							    ...
							    <option value="2196013">2196013</option>
							    <option value="2196053">2196053</option>
							    <option value="2420421">2420421</option>
							    <option value="2486844">2486844</option>
							    <option value="2159030">2159030</option>
						    </select>
						</td>
						<td align="left">
					...
					<tbody><tr>
					   <td>ხელმისაწვდომია<br><br>
					    <select size="10" name="List_Agent_available" multiple="multiple" id="myform_List_Agent_from" style="height: 100px;width: 125px;">
							...
							<option value="EXT200">EXT200</option>
							<option value="EXT201">EXT201</option>  
						</select>
					</td>
					<td align="left">
			...
			<div id="rest" style="margin-top: 200px; width: 100%; float:none;">
                ...
                <tr>
                    ...
                    <th style="background: #F8BB2C; color: #FFFFFF">გამოტოვებული</th>
                ...
                <tr id="technik_info">
                    ...
                    <td id="miss_dialog" style="cursor: pointer; text-decoration: underline;"></td>
                    <td ></td>
      ...
      <div style="margin-top: 290px; position: absolute; left: 570px;"><button id="excel_answer_service_level_info">Excel</button></div>
		...
		<table width="99%" cellpadding="1" cellspacing="1" border="0" class="sortable" id="table1">
		 ...
		 <div id="tab-4">
		      <div style="margin-top: 55px; position: absolute; right: 30px;"><button id="excel_tech_info_miss">Excel</button></div>
                <table width="100%" border="0" cellpadding="0" cellspacing="0" style="margin-top: 50px">
                <caption>ტექნიკური ინფორმაცია</caption> 
                <tbody>
                <tr>
                ...
                <tr id="technik_miss_info">
                    <td>ზარი</td>
                    <td></td>
                    <td id="miss_answear_dialog" style="cursor: pointer; text-decoration: underline;"></td>
                    <td id="miss_unanswear_dialog" style="cursor: pointer; text-decoration: underline;"></td>
                    <td id="miss_undone_dialog" style="cursor: pointer; text-decoration: underline;"></td>                    
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                ...
                <div id="chart_container0" style="float:left;" width: 50%; height: 300px;"></div>
                 <div id="chart_container0r" style="float:left;" width: 50%; height: 300px;"></div>
                 <br>
                <div style="margin-top: 5px; position: absolute; right: 40px;"><button id="excel_answer_call_by_agent_info_miss">Excel</button></div>
                ...
                <caption>ნაპასუხები ზარები ოპერატორების მიხედვით</caption>
                    ...
                    <tbody id="answer_call_by_queue_miss">
                        
        			</tbody>
                </table>
                <br>
                <div style="margin-top: 580px; position: absolute; left: 570px;"><button id="excel_answer_call_by_queue_info_miss">Excel</button></div>
                <table width="49%" cellpadding="3" cellspacing="3" border="0" style="float:left;">
                    <caption>ნაპასუხები ზარები რიგის მიხედვით</caption>
                    <thead>
                    <tr>
                    <td valign="top" width="100%" bgcolor="#fffdf3">
                        ...
                        <thead>
                        <tr> 
                            <th><a  class="sortheader" onclick="">რიგი<span class="sortarrow">&nbsp;&nbsp;&nbsp;</span></a></th>
                            <th><a  class="sortheader" onclick="">სულ ნაპასუხები<span class="sortarrow">&nbsp;&nbsp;&nbsp;</span></a></th>
                            <th><a  class="sortheader" onclick="">სულ ნაპასუხები %<span class="sortarrow">&nbsp;&nbsp;&nbsp;</span></a></th>
                        </tr>
                        </thead>
                        <tbody id="answer_call_miss">
                    ...
                    <div style="margin-top: 580px; position: absolute; left: 570px;"><button id="excel_unanswer_call_by_queue_info_miss">Excel</button></div>
                    <table width="49%" cellpadding="3" cellspacing="3" border="0" style="float:left; margin-left: 25px;">
                        <caption>უპასუხო ზარები რიგის მიხედვით</caption>
                        <thead>
                        <tr>
                        <td valign="top" width="100%" bgcolor="#fffdf3">
                            ...
                            <tr> 
                                <th><a  class="sortheader" onclick="">რიგი<span class="sortarrow">&nbsp;&nbsp;&nbsp;</span></a></th>
                                <th><a  class="sortheader" onclick="">სულ უპასუხო<span class="sortarrow">&nbsp;&nbsp;&nbsp;</span></a></th>
                                <th><a  class="sortheader" onclick="">სულ უპასუხო %<span class="sortarrow">&nbsp;&nbsp;&nbsp;</span></a></th>
                            </tr>
                            </thead>
                            <tbody id="unanswer_call_miss">
...
<!-- jQuery Dialog -->
<div id="add-edit-form-miss" class="form-dialog" title="გამოტოვებული">
<div id="test"></div>
</div>

<div id="add-edit-form-undone" class="form-dialog" title="დაუმუშავებული">
<div id="test"></div>
</div>
