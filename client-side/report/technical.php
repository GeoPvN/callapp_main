...
<meta http-equiv="content-type" content="application/vnd.ms-excel; charset=UTF-8">
	<style type="text/css">
		...
		#call_distribution_per_day  td:nth-child( 2 ),
		#call_distribution_per_day  td:nth-child( 4 ),
	...
	<script type="text/javascript">
		...
		function getData7(){
		 ...
		 function drawFirstLevel1(){
			   ...
			   start_time = $('#start_time').val();
			   end_time = $('#end_time').val();
			              $.getJSON("server-side/report/tab_0_0.action.php?start="+start_time + "&end=" + end_time + "&agent=" + agent + "&queuet=" + queuet, function(json) {
			                  options.series[0].data = json;
			                  chart = new Highcharts.Chart(options);
			              });
			   }
		...
		$(document).on("click", "#call_distribution_per_day  td:nth-child( 2 )", function() {
			var firstvar = $($(this).siblings()[0]).text();
		    NEWDIALOG('answear_dialog',firstvar,'new_dealog_table',7);
		});
		$(document).on("click", "#call_distribution_per_day  td:nth-child( 4 )", function() {
			var firstvar = $($(this).siblings()[0]).text();
			NEWDIALOG('unanswear_dialog',firstvar,'new_dealog_table_1',5);
		});

		$(document).on("click", "#call_distribution_per_hour  td:nth-child( 2 )", function() {
			var firstvar = $($(this).siblings()[0]).text();
		    NEWDIALOG('answear_hour_dialog',firstvar,'new_dealog_hour_table',7);
		});
		$(document).on("click", "#call_distribution_per_hour  td:nth-child( 4 )", function() {
			var firstvar = $($(this).siblings()[0]).text();
			NEWDIALOG('unanswear_hour_dialog',firstvar,'new_dealog_hour_table_1',5);
		});

		$(document).on("click", "#call_distribution_per_day_of_week  td:nth-child( 2 )", function() {
			var firstvar = $($(this).siblings()[0]).text();
		    NEWDIALOG('answear_dayofweek_dialog',firstvar,'new_dealog_dayofweek_table',7);
		});
		$(document).on("click", "#call_distribution_per_day_of_week  td:nth-child( 4 )", function() {
			var firstvar = $($(this).siblings()[0]).text();
					...
					<tr>
					   	<td>
						    ...
						    <select name="List_Queue_available" multiple="multiple" id="myform_List_Queue_from" size="10" style="height: 100px;width: 125px;" >
							    <option value="2200404">2200404</option>
							    <option value="2404004">2404004</option>				    
							</select>
						</td>
						<td align="left">
					...
					<tbody><tr>
					   <td>ხელმისაწვდომია<br><br>
					    <select size="10" name="List_Agent_available" multiple="multiple" id="myform_List_Agent_from" style="height: 100px;width: 125px;">
							<option value="Call Center 1">Call Center 1</option>
							<option value="Call Center 2">Call Center 2</option>
							<option value="Call Center 3">Call Center 3</option>
							<option value="Call Center 4">Call Center 4</option>
						</select>
					</td>
					<td align="left">
