...
.quest_body{
	...
	<script type="text/javascript">
	    var gela = {};
	    var next_i = 0;
		var aJaxURL		= "server-side/call/outgoing/new/outgoing_tab0.action.php";		//server side folder url
		var aJaxURL1	= "server-side/call/outgoing/new/outgoing_tab1.action.php";		//server side folder url
		var aJaxURL2	= "server-side/call/outgoing/new/outgoing_tab2.action.php";		//server side folder url
		var aJaxURL3	= "server-side/call/outgoing/new/outgoing_tab3.action.php";		//server side folder url
		var aJaxURL4	= "server-side/call/outgoing/new/outgoing_tab4.action.php";		//server side folder url
		var aJaxURL7	= "server-side/call/outgoing/new/outgoing_tab7.action.php";		//server side folder url
		var aJaxURL5	= "server-side/call/outgoingnew//suboutgoing/outgoing_tab1.action.php";		//server side folder url
		var aJaxURL6	= "server-side/call/outgoing/new/suboutgoing/outgoing_tab2.action.php";		//server side folder url
		var aJaxURL8	= "server-side/call/outgoing/new/outgoing_tab8.action.php";		//server side folder url
		...
		function GetTable0() {
            LoadTable0();
            SetEvents("", "", "", "example0", "add-edit-form", aJaxURL); 
        }
		...
		function LoadTable5(){		
			...
			/* Table ID, aJaxURL, Action, Colum Number, Custom Request, Hidden Colum, Menu Array */
			GetDataTableproduct("sub1", aJaxURL5, "get_list&scenar_name="+scenar_name, 5, "", 0, "", 1, "asc", "");
		...
		function LoadTable6(){		
			...
			/* Table ID, aJaxURL, Action, Colum Number, Custom Request, Hidden Colum, Menu Array */
			GetDataTableproduct("sub2", aJaxURL6, "get_list&scenar_name="+scenar_name, 5, "", 0, "", 1, "asc", "");
		...
		$(document).on("click", "#add_button", function () {
			var buttons = {
				"save": {
		            ...
		            click: function () {
		    			...
		    			if(param.task_type_id == 10){
		    		 		if(param.template_id == 0){
		    			 		alert('ამოირჩიეთ სცენარი');
		    		 		}else{
		    			 		$.ajax({
		    			        url: "server-side/call/outgoing/new/tasks_tab0.action.php",
		    				    data: param,
			...
			$.ajax({
		        ...
		        success: function(data) {       
					$("#task_dialog").html(data.page);
					GetDateTimes("done_start_time");
					GetDateTimes("done_end_time");
					$("#choose_base").button();
					GetDataTableTest("example4", "server-side/call/outgoing/new/tasks_tab0.action.php", "get_list&id="+$("#id").val(), 12, "", 0, "", 1, "asc", "");
			    }
		    });						
		});
		        ...
		        success: function(data) {       
					if(typeof(data.error) != "undefined"){
						if(data.error != ""){
							...
							SetEvents("", "", "check-all-base", "base", "phone_base_dialog", "server-side/call/outgoing/new/tasks_tab0.action.php");
							GetDataTableTest("base", "server-side/call/outgoing/new/tasks_tab0.action.php", "get_list_base_phone", 11, "", 0, "", 1, "asc", "");
						}
					}
			    }
			});
	    ...
	    $(document).on("click", "#incomming_base", function () {
			$("#hidden_base").val('1');
			SetEvents("", "", "check-all-base", "base", "phone_base_dialog", "server-side/call/outgoing/new/tasks_tab0.action.php");
			GetDataTableTest("base", "server-side/call/outgoing/new/tasks_tab0.action.php", "get_list_base", 11, "", 0, "", 1, "asc", "");
	    ...
	    $(document).on("click", "#phone_base", function () {
			$("#hidden_base").val('2');
			SetEvents("", "", "check-all-base", "base", "phone_base_dialog", "server-side/call/outgoing/new/tasks_tab0.action.php");
			GetDataTableTest("base", "server-side/call/outgoing/new/tasks_tab0.action.php", "get_list_base_phone", 11, "", 0, 0, 1, "asc", "");
		...
		$(document).on("change", "#dialog-form .check_input", function () {
			$("#next_quest").attr('next_id', $(this).val());
		});

		$(document).on("change", "#dialog-form .radio_input", function () {			
			$("#next_quest").attr('next_id', $(this).val());
		});

		$(document).on("change", "#dialog-form .inputtext", function () {			
			$("#next_quest").attr('next_id', $(this).attr('q_id'));
		});
		
		$(document).on("change", "#dialog-form .date_input", function () {			
			$("#next_quest").attr('next_id', $(this).attr('q_id'));
		});
		
		$(document).on("change", "#dialog-form .date_time_input", function () {			
			$("#next_quest").attr('next_id', $(this).attr('q_id'));
		});
        ...
        $(document).on("click", "#next_quest", function () {        	
        	$.ajax({
                url: aJaxURL1,
                type: "POST",
                data: "act=next_quest&id=" + $("#id").val() + "&next_id=" + $(this).attr('next_id'),
                dataType: "json",
                success: function (data) {
                    if (data.error != "") {
                        alert(data.error);
                    	...
                    	$("#back_quest").css('display', 'initial');
                    	for(var u=0; u < $('.quest_body').length; u++){
            	            if(gela[u] == data.ne_id){
            	            	next_i = u;            	            	
            	                $("#"+gela[u]).css('display', 'initial');   
            	                if(($('.quest_body').length-1)==u){
           	             		 $("#next_quest").css('display', 'none');
            	             	}
            	            }
            	        }
                    ...
                    if(data.ne_id == null){
                    	$("#"+$("#next_quest").attr('next_id')).css('display', 'initial'); 
                    }
                }
            });        	
        });
        ...
        $(document).on("click", "#back_quest", function () {        	
        	$("#next_quest").css('display', 'initial');
        	next_i--;
        	$(".quest_body").css('display', 'none');
     		$("#"+gela[next_i]).css('display', 'initial');        	
        	if(0==next_i){
        		$("#back_quest").css('display', 'none');
        		$("#next_quest").css('display', 'initial');
        	}
	 ...
	 function GetProductionInfoo(name) {
	    ...
	    $(document).on("click", "#sub1 tr td", function () {
	    ...
	    $(document).on("click", "#sub2 tr td", function () {
			if ($(this).closest("tr").find("input:checkbox").is(':checked')) {
			    var chk = $(this).closest("tr").find("input:checkbox").prop('checked', false);
			}else{
				var chk = $(this).closest("tr").find("input:checkbox").prop('checked', true);
	    ...
	    $(document).on("click", "#done-dialog1", function () {
	    	...
	    	$('#dialog-form .check_input:checked').each(function() {	
		    	
	    		key      = this.name;
	    		value    = this.value;
	    		ansver_val    = $(this).attr('ansver_val');
	    		
	    		checker[key] = checker[key] + "," + value  + "^" + ansver_val;

	    	});
	    	
	    	items.checker = checker;
	    	
	        $('#dialog-form .inputtext').each(function() {	
	    	    ...
	    	    if(inp_value != ''){
	    		 inp_checker[inp_key] = inp_checker[inp_key] + "," + inp_value + "^" + inp_q_id;
	    	    }
	    	});
	    	
	    	items.input   = inp_checker;

	        $('#dialog-form .prod_inp:checked').each(function() {	
		    	
	        	prod_key      = this.name;
	    		prod_value    = this.value;
	    		ansver_val    = $(this).attr('ansver_val');

	    		prod_checker[prod_key] = prod_checker[prod_key] + "," + prod_value  + "^" + ansver_val;

	    	});
	    	
	    	items.prod    = prod_checker;		
	        ...
	        $('#dialog-form .radio_input:checked').each(function() {	
	    		...
	    		radio_checker[radio_key] = checker[radio_key] + "," + radio_value + "^" + ansver_val;

	    	});
	    	
	    	items.radio = radio_checker;	

	        $('#dialog-form .date_input').each(function() {	
	        ...
	        $('#dialog-form .date_time_input').each(function() {	
		    ...
		    $('#dialog-form .prod_count').each(function() {	
	    	    if(this.value > 0){
	    	    	param.prod_count = this.value;
	    	    }
	    	});
			...
			var link = GetAjaxData(param);
 	    	...
 	    	$.ajax({
 		        url: aJaxURL1,
 			    data: link + "&checker=" + JSON.stringify(items.checker) + "&input=" + JSON.stringify(items.input) + "&prod=" + JSON.stringify(items.prod) + "&radio=" + JSON.stringify(items.radio) + "&date=" + JSON.stringify(items.date) + "&date_time=" + JSON.stringify(items.date_time),
 		        success: function(data) {       
 					if(typeof(data.error) != "undefined"){
 						if(data.error != ""){
 							alert(data.error);
...
<!-- jQuery Dialog -->
<div id="task_dialog" class="form-dialog" title="დავალების ფორმირება">
<!-- aJax -->
</div>
<div id="phone_base_dialog" class="form-dialog" title="დავალების ფორმირება">
<!-- aJax -->
</div>
<div id="add_formireba_dialog" class="form-dialog" title="დავალების ფორმირება">
<!-- aJax -->
</div>
