	...
	<script type="text/javascript">
			...
			$("#pages tbody").on("dblclick", "tr", function () {
		        ...
		        if (empty != "dataTables_empty") {
		            ...
		            $.ajax({
		                url: aJaxURL,
		                type: "POST",
		                data: "act=get_notes&id=" + rID + "&shab_id=" + shab_id,
		                dataType: "json",
		                    ...
		                    if (typeof (data.error) != "undefined") {
		                        if (data.error != "") {
		                            ...
		                            if ($.isFunction(window.LoadDialog)) {
		                            	...
		                            	GetButtons("del_product");
		                            	GetDialog("add-group-form", 1000, "auto", buttons);
		                            	var filter 	= $("#hidden_id").val();
		                            	var shab_id = $("#shab_id").val();
		                            	var group_name = $("#group_name").val();
		                            	GetDataTable("example1", aJaxURL, "get_list_product&filter="+filter+"&shab_id="+shab_id+"&group_name="+group_name, 8, "", 0, "", 1, "desc");
		                            }
		                        }
		                    }
		                }
		...
		$(document).on("click", "#del_product", function () {
			 ...
			 for (var i = 0; i < data.length; i++) {
 		            $.ajax({
 		                url: aJaxURL,
 		                type: "POST",
 		                data: "act=disable_prod&id_del_prod=" + data[i],
 		                dataType: "json",
 		                success: function (data) {
 		                	var filter 	= $("#hidden_id").val();
                        	var group_name = $("#group_name").val();
                        	GetDataTable("example1", aJaxURL, "get_list_product&filter="+filter+"&group_name="+group_name, 8, "", 0, "", 1, "desc");
 		                }
 		            });
		        }
		});
			...
			if(checker_id == 0){
	        	...
	        	//Action
	 			...
	 			var scenar_id 	= $("#scenar_id").val();
	 	    	    ...
	 	    	    $.ajax({
	 	    	        url: aJaxURL,
	 	    		    data: "act="+act+"&minishneba="+minishneba+"&qvota="+qvota+"&hidden_id="+hidden_id+"&group_name="+group_name+"&scenar_id="+scenar_id+"&shab_id="+shab_id,
	 	    	        success: function(data) {
	 	    				if(typeof(data.error) != "undefined"){
	 	    					if(data.error != ""){
	 	    						alert(data.error);
		...
		$(document).on("click", "#save-dialog", function () {
			...
			var val1;
 			...
 			if( param.nam == "" ){
 				alert("შეიყვანეთ  სცენარის სახელი!");
 			}else{
 	    	    $.ajax({
 	    	        url: aJaxURL,
 	    		    data: param,
