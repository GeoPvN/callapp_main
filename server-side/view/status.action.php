...
function Adddepartment($department_id, $department_name, $type)
{
	...
	mysql_query("INSERT INTO 	 `task_status`
								(`name`,`user_id`,`type`)
...
{
	$user_id	= $_SESSION['USERID'];
	mysql_query("	UPDATE `task_status`
...
function GetPage($res = '')
{
	...
	<div id="dialog-form">
	    ...
	    if($res['type'] == 1){
	        $data .= '<option value="0">----</option><option value="1" selected="selected">გამავალი</option><option value="2">დავალება</option>';
	    }elseif($res['type'] == 2){
	        $data .= '<option value="0">----</option><option value="1">გამავალი</option><option value="2" selected="selected">დავალება</option>';
	    }else{
	        $data .= '<option value="0" selected="selected">----</option><option value="1">გამავალი</option><option value="2" >დავალება</option>';
	    }
						$data .= '</select>
					</td>
				</tr>
