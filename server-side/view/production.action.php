		...
		if($priority_name != ''){
				if ($priority_id == '') {
					mysql_query("INSERT INTO 	 	`production`
									(`user_id`,`name`,`genre_id`,`production_category_id`,`comment`,`description`,`price`,`unicid`,`buy_pirce` )
					VALUES 		('$user_id','$priority_name','$genre_id','$production_category_id','$_REQUEST[comment]','$_REQUEST[description]','$_REQUEST[price]','$_REQUEST[unicid]','$_REQUEST[buy_pirce]')");
...
function Getpriority($priority_id)
{
	$res = mysql_fetch_assoc(mysql_query("	SELECT 	`production`.`id`,
													...
													`genre`.`id` AS `genre`,
													`department`.`id` AS `production_category`,
													`production`.`price`,
	                                                `production`.`unicid`,
	                                                `production`.`buy_pirce`
											FROM 	`production`
	...
	<div id="dialog-form">
	    <fieldset>
	    	...
	    	<table class="dialog-form-table">
				...
				<tr>
					<td style="width: 170px;"><label for="buy_pirce">შესყიდვის ფასი</label></td>
					<td>
						<input type="text" id="buy_pirce" class="idle address" onkeypress="{if (event.which != 8 && event.which != 0 && event.which!=46 && (event.which < 48 || event.which > 57)) {$(\'#errmsg\').html(\'მხოლოდ რიცხვი\').show().fadeOut(\'slow\'); return false;}}" value="' . $res['buy_pirce'] . '" />
						<span id="errmsg" style="color: red;"></span>
					</td>
				</tr>
				<tr>
					<td style="width: 170px;"><label for="CallType">განყოფილება</label></td>
					<td>
				...
				<tr>
					<td style="width: 170px;"><label for="unicid">შტრიხკოდი</label></td>
					<td>
						<input type="text" id="unicid" class="idle address" value="' . $res['unicid'] . '" />
					</td>
				</tr>
