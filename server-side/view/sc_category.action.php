    ...
    case 'get_edit_page':
 	...
 	case 'get_list' :
	    ...
	    $rResult = mysql_query("SELECT	`info`.`id`,
                                		`info`.`name`,
                                		(SELECT `name` FROM `scenario_category` WHERE `id` = `info`.`parent_id`)
							    FROM	`scenario_category` AS `info`
	    						WHERE	`actived` = 1");
...
{
	mysql_query("INSERT INTO `scenario_category`
					(`name`, `parent_id`) 
				 VALUES
					('$cat_name', $par_id)");
}
...
{
    mysql_query("UPDATE `scenario_category`
				 SET    `actived` = 0
				 WHERE	`id` = $cat_id");
}
