...
switch ($action) {
	...
	case 'save_notes':
		...
		$shab_id		= $_REQUEST['shab_id'];
		
		if($shab_id !=''){
		    if($hidden_id==3 or $hidden_id == 4){
		    mysql_query("UPDATE   `shabloni` SET
            		              `notes`='$minishneba'
            		     WHERE    `quest_id`='$hidden_id' AND `name`= '$group_name'");
		    }else{
		        mysql_query("UPDATE   `shabloni` SET
            		                  `notes`='$minishneba'
            		        WHERE     `id`='$shab_id'");
	...
	case 'get_notes':
	    ...
	    $group_id		= $_REQUEST['id'];
	    $shab_id		= $_REQUEST['shab_id'];
	    $page		    = GetNotes($group_id,$shab_id);
	...
	case 'get_list_product':
		...
		if($shab_id != ''){
		    $rResult = mysql_query("SELECT 	`shabloni`.`id`,
		        ...
		        WHERE 	`production`.`actived`=1 AND shabloni.quest_id = '$filter' AND shabloni.`name` = '$group_name'");
		}else{
		$rResult = mysql_query("SELECT 	`shabloni`.`id`,
		                                `production`.`unicid`,
										`production`.`name`,
	...
	case 'get_pages_list':
		...
		$group_name = $_REQUEST['group_name'];
			...
			$rResult = mysql_query("SELECT          	`quest`.`id`,
								          		        `quest`.`name`,
											            `shabloni`.`id` as `shabl`
									FROM      	        `quest`
									left JOIN	        `shabloni` ON `quest`.`id` = `shabloni`.`quest_id` AND `shabloni`.`name` = '$group_name'
		...
		while ( $aRow = mysql_fetch_array( $rResult ) )
		{
			...
			for ( $i = 0 ; $i < $count ; $i++ )
			{
				...
				if($i == ($count - 1)){
					...
					$row[] = '<input type="checkbox" name="check_' . $aRow[$hidden] . '" class="check1" value="' . $aRow[$hidden] . '" '.$check.'/><input style="display:none;" id="shab_id" type="text" value="' . $aRow['shabl'] . '">';
				}
	...
	case 'save_group':
		...
		if($group_id != ''){
		    $full_edit_res = mysql_fetch_row(mysql_query("SELECT `name`
        							      FROM 	shabloni 
        		                          WHERE id = '$group_id'"));
		    
		    mysql_query("UPDATE `shabloni` SET 
                                `name`='$group_name'
                        WHERE   `name`='$full_edit_res[0]'");
		}
...
function GetProductSearch($title){
	$res = mysql_fetch_assoc(mysql_query("SELECT 	`production`.`id`,
													...
													`department`.`name` AS `category_name`,
...
function GetNotes($id, $shab_id){
    $shabl = mysql_fetch_assoc(mysql_query(" SELECT 	`id`,
                                        				`scenar_id`,
                                        				`quest_id`,
                                        				`name`,
                                        				`notes`,
	...
	if(($id < 8) or ($id == 21)){
		...
		<div id="dialog-form">
	 	    <fieldset>
				...
				<table>
				<tr>
		    	<td><textarea  style="width: 940px; height:60px; resize: none;" id="minishneba" class="idle" name="content" cols="300" >'.$shabl[notes].'</textarea></td>
				</tr>
				</table>
	        </fieldset>
		...
		<div id="dialog-form">
			...
			<fieldset>
				...
				<div id="button_area" >
		    	    ...
		    	    <button id="del_product">წაშლა</button>
        		</div>
				<table class="display" id="example1" >
                    <thead >
                        <tr id="datatable_header">
                            <th>ID</th>
                            <th style="width: 20%;">პროდუქტი</th>
		    	            <th style="width: 20%;">შტრიხკოდი</th>
                            <th style="width: 20%;">ჟანრი</th>
	...
	$data .='<input type="text" id="full_edit_id" class="idle" onblur="this.className=\'idle\'" style="display:none;" value="'.$_REQUEST['group_id'].'"/>
	         <input type="text" id="edit_id" class="idle" onblur="this.className=\'idle\'" style="display:none;" value="'.$_REQUEST['shab_id'].'"/>';
	return $data;
}
