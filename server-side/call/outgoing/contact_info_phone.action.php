	...
	case 'get_list':
		...
		$hidden = 		$_REQUEST['hidden'];
	  	...
	  	$rResult = mysql_query("SELECT  `id`,
                                ...
                                WHERE   `actived` = 1 AND `type` = 1 AND outgoing_campaign_detail_contact_id = $_REQUEST[outgoing_campaign_detail_contact_id]");
    ...
    case 'save_contact_info_phone':
        ...
        if($_REQUEST['outgoing_campaign_detail_contact_detail_id'] == ''){
            mysql_query("INSERT INTO `outgoing_campaign_detail_contact_detail`
                        ...
                        VALUES
                        ('$outgoing_campaign_detail_contact_id', NOW(), '$_SESSION[USERID]', '1', '$_REQUEST[contact_info_phone]');");
        }else{
            mysql_query("UPDATE `outgoing_campaign_detail_contact_detail` SET 
                                `date`=NOW(),
                                `user_id`='$_SESSION[USERID]',
                                `name`='$_REQUEST[contact_info_phone]'
	...
	<div id="dialog-form">
	    <fieldset>
	       <legend>ძირითადი ინფორმაცია</legend>
               ...
               <table>
                   <tr>
                       <td><label for="contact_info_phone">ტელეფონი</label></td>
                   </tr>
                   <tr>
                       <td><input id="contact_info_phone" type="text" value="'.$res[name].'"></td>
                   </tr>
               </table>
	    </fieldset>
	</div>';
