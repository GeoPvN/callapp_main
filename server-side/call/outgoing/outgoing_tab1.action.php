    ...
    case 'get_edit_page':
 	...
 	case 'get_list' :
	    ...
	    if($user == 1  || $user == 3){
	    	...
	    	$user_checker = 'and responsible_user_id='.$user;
	    }
	    ...
	    $columns = array(
	        ...
	        array( 'db' => 'addres',  			'dt' => 2 ),
	        array( 'db' => 'start_date',        'dt' => 3 ),
	        array( 'db' => 'end_date',     		'dt' => 4 ),
	        array( 'db' => 'task_type_name',	'dt' => 5 ),
	        array( 'db' => 'shabloni_name',		'dt' => 6 ),
	        array( 'db' => 'first_last_name',	'dt' => 7 ),
	        array( 'db' => 'person_name',		'dt' => 8 ),
	        array( 'db' => 'prio_name',			'dt' => 9 ),
	        array( 'db' => 'note',		        'dt' => 10 ),
	    ...
	    */
	    ...
	    $where_param = "where `status` = 1 $user_checker ";
        ...
        case 'done_outgoing':
			...
			$hello_comment1		= $_REQUEST['hello_comment1'];
			...
			$result_quest1		    = $_REQUEST['result_quest1'];
			$result_comment1		= $_REQUEST['result_comment1'];
			...
			if($set_task_department_id != 0){
				TaskFromireba($set_task_department_id, $set_persons_id, $set_priority_id, $set_start_time, $set_done_time, $set_body, $task_type_id_seller, $set_shabloni, $task_detail_id);
			}
			
			
			if($result_quest == 1){	
			    $cheker_sale = mysql_num_rows(mysql_query("SELECT id FROM `elva_sale_new` WHERE task_id = $task_detail_id"));
			    if($cheker_sale == 0){			
				SaveElvaGe($person_n, $first_name, $mail, $addres, $phone, $phone1, $send_date, $call_content,'','',$task_detail_id );
			    }else{
				SaveElvaGeUp($person_n, $first_name, $mail, $addres, $phone, $phone1, $send_date, $call_content,'','',$task_detail_id );
			    }
			}
			...
			if($res_cheker[0]==''){
        		Savetask1($task_detail_id, $hello_quest, $hello_comment, $info_quest, $info_comment, $result_quest, $result_comment, $payment_quest, $payment_comment, $send_date, $preface_name, $preface_quest, $d1, $d2, $d3, $d4, $d5, $d6, $d7, $d8, $d9, $d10, $d11, $d12, $q1, '', $get_gift, $b1, $b2, $result_quest1, $result_comment1, $hello_comment1);
			}else{			    
				Savetask1Up($status, $task_detail_id, $hello_quest, $hello_comment, $info_quest, $info_comment, $result_quest, $result_comment, $payment_quest, $payment_comment, $send_date, $preface_name, $preface_quest, $d1, $d2, $d3, $d4, $d5, $d6, $d7, $d8, $d9, $d10, $d11, $d12, $q1, '', $get_gift, $b1, $b2, $result_quest1, $result_comment1, $hello_comment1);
			}
...
function Savetask1Up($status, $task_detail_id, $hello_quest, $hello_comment, $info_quest, $info_comment, $result_quest, $result_comment, $payment_quest, $payment_comment, $send_date, $preface_name, $preface_quest, $d1, $d2, $d3, $d4, $d5, $d6, $d7, $d8, $d9, $d10, $d11, $d12, $q1, $get_prod, $get_gift, $b1, $b2, $result_quest1, $result_comment1, $hello_comment1)
{
    $user  = $_SESSION['USERID'];
    $c_date		= date('Y-m-d H:i:s');
    $xelo = '';
    if($hello_comment == ''){
        $xelo = $hello_comment1;
    }elseif($hello_comment1 == ''){
        $xelo = $hello_comment;
    }
    
    $res_status = mysql_fetch_row(mysql_query(" SELECT  `status`
                                                FROM    `task_detail`
                                                WHERE   `id`='$task_detail_id'"));
    $c_date_check = '';
    if($status == $res_status[0]){
        $c_date_check = '';
    }else{
        $c_date_check = "`date`='$c_date',";
    }
	...
	mysql_query("
				UPDATE `task_scenar` SET 
						...
						`b1`='$b1', 
						`b2`='$b2',
						`result_quest1`='$result_quest1', 
						`result_comment1`='$result_comment1'
				WHERE `task_detail_id`='$task_detail_id'
				");
}

function SaveElvaGe($person_n, $first_name, $mail, $addres, $phone, $phone1, $send_date, $result_comment,$get_prod_row, $row_name_prod, $task_id)
{
	$user  = $_SESSION['USERID'];
	$c_date		= date('Y-m-d H:i:s');
	
	mysql_query("
			INSERT INTO `elva_sale_new`
			(`person_id`, `name_surname`, `mail`, `address`, `phone`, `phone1`, `period`, `books`, `call_date`, `sum_price`, `callceenter_comment`, `operator_id`, `oder_send_date`, `task_id`, `custom_prod`,`cash`,`status`)
			VALUES
			('$person_n', '$first_name', '$mail', '$addres', '$phone', '$phone1', '$send_date', '$row_name_prod', '$c_date', '$get_prod_row', '$result_comment', '$user', '', '$task_id', '',0,0)
				");
}

function SaveElvaGeUp($person_n, $first_name, $mail, $addres, $phone, $phone1, $send_date, $call_content,$get_prod_price,$row_name_prod,$SaveElvaId )
{
    mysql_query("UPDATE `elva_sale_new` SET 
                        `person_id`='$person_n',
                        `name_surname`='$first_name',
                        `mail`='$mail',
                        `address`='$addres',
                        `phone`='$phone',
                        `phone1`='$phone1',
                        `period`='$send_date',
                        `books`='$row_name_prod',
                        `sum_price`='$get_prod_price',
                        `callceenter_comment`='$call_content'
                  WHERE `task_id`='$SaveElvaId'");
}
...
function Savetask1($task_detail_id, $hello_quest, $hello_comment, $info_quest, $info_comment, $result_quest, $result_comment, $payment_quest, $payment_comment, $send_date, $preface_name, $preface_quest, $d1, $d2, $d3, $d4, $d5, $d6, $d7, $d8, $d9, $d10, $d11, $d12, $q1, $get_prod, $get_gift, $b1, $b2, $result_quest1, $result_comment1, $hello_comment1)
{
    $xelo = '';
    if($hello_comment == ''){
        $xelo = $hello_comment1;
...
VALUES
('$user', '$cur_date', '$task_detail_id', '$xelo', '$hello_quest', '$info_comment', '$info_quest', '$result_comment', '$result_quest', '$send_date', '$payment_comment', '$payment_quest', '$preface_name', '$preface_quest', '$d1', '$d2', '$d3', '$d4', '$d5', '$d6', '$d7', '$d8', '$d9', '$d10', '$d11', '$d12', '$q1', '$get_prod', '$get_gift', '$b1', '$b2', '$result_quest1', '$result_comment1')
	");
