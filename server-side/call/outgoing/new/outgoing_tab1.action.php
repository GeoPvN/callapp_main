...
switch ($action) {
    ...
    case 'get_edit_page':
        ...
        break;
    case 'next_quest':
        $page 		= next_quest($task_id, $_REQUEST[next_id]);
 	...
 	case 'get_list' :
	    ...
	    */
        ...
        case 'done_outgoing':
			...
			$upOrInsert = mysql_fetch_array(mysql_query("  SELECT task.template_id
                                                           FROM `task_detail`
                                                           JOIN task ON task_detail.task_id = task.id
                                                           WHERE task_detail.id  = '$task_detail_id'"));
			
			
			if($upOrInsert[0] > 0){
			    $result = mysql_num_rows(mysql_query("SELECT id FROM scenar_$upOrInsert[0] WHERE task_detail_id = $task_detail_id"));
			    if($result == 0){
    			    mysql_query("INSERT INTO `scenar_$upOrInsert[0]`
    			                 (`task_detail_id`)
    		                     VALUES
    			                 ($task_detail_id);");
			    }
			...
			$res = mysql_query("select * from scenar_$upOrInsert[0]");	
		    ...
		    for ( $i = 0; $i < $field; $i++ ) {
		        ...
		        if($i > 1){
		            mysql_query("UPDATE `scenar_$upOrInsert[0]` SET
    			                        `$names`=''
			...
			foreach ($checker as $key => $value) {
                ...
                $g = 1;
                $r = 0;
			    for($i = 1;$i <= (count((array)$last)/2);$i++){				            
			        mysql_query("UPDATE `scenar_$upOrInsert[0]` SET
                                        `answer_$last[$r]`='$last[$g]'
                                 WHERE  `task_detail_id` = $task_detail_id;");
			        $g+=2;
			        $r+=2;			        
 			    }
			...
			foreach ($radio as $key => $value) {
			    ...
			    mysql_query("UPDATE `scenar_$upOrInsert[0]` SET
                                    `answer_$last[0]`='$last[1]'
                             WHERE  `task_detail_id` = $task_detail_id;");
			}

			foreach ($input as $key => $value) {
			    $quest_id          = preg_split("/[\,|]+/",$key);
			    $quest_detail_id   = preg_split("/[\,|]+/",$key);
			    $val               = substr($value,10);	
			    $or = explode('^',$val,2);	
			    $value_id   = $or[1];
			    $value_text = rtrim($or[0]);	
                mysql_query("UPDATE `scenar_$upOrInsert[0]` SET
                                    `answer_$value_id`='$value_text'
                             WHERE  `task_detail_id` = $task_detail_id;");
			}
			...
			foreach ($date as $key => $value) {
			    ...
			    $val               = substr($value,10);
 			    ...
 			    mysql_query("UPDATE `scenar_$upOrInsert[0]` SET
                                    `answer_$quest_id[2]`='$val'
                             WHERE  `task_detail_id` = $task_detail_id;");
			}
			
			foreach ($date_time as $key => $value) {
			    $quest_id          = preg_split("/[\s,|]+/",$key);
			    $quest_detail_id   = preg_split("/[\s,|]+/",$key);
			    $val               = substr($value,10);
...
// 			    mysql_query("UPDATE `scenar_$upOrInsert[0]` SET
//             			            `quest_$quest_id[1]`='$last[0]',
//             			            `answer_$quest_id[1]`='$last[1]'
//             			     WHERE  `task_detail_id` = $task_detail_id;");
			    
			    $val = str_replace("^",",",$val);
			    $twoinone = preg_split("/[\s,]+/",$val);
			    foreach ($twoinone as $twoinone_val){
			     $get_gift_chek .= ", '".$twoinone_val."'";
			    }			    
			}
			...
			if(count((array)$prod) != 0){
			    ...
			    if($SaveElvaChek[0] == ''){
			        ...
			        SaveElvaGe($person_n, $first_name, $mail, $addres, $phone, $phone1, $send_date, $call_content,$get_prod_price[0],substr($row_name_prod,0,-3),$task_id );
			    }else{
			        $get_prod = substr($get_gift_chek, 1);
			        $get_prod_price = mysql_fetch_row(mysql_query("SELECT SUM(price)
			            FROM `production`
			        ...
			        SaveElvaGeUp($person_n, $first_name, $mail, $addres, $phone, $phone1, $send_date, $call_content,($get_prod_price[0]*$_REQUEST[prod_count]),substr($row_name_prod,0,-3),$SaveElvaChek[0] );
			    }
			}else{
...
function SaveElvaGe($person_n, $first_name, $mail, $addres, $phone, $phone1, $send_date, $result_comment,$get_prod_row, $row_name_prod, $task_id)
{
	...
	mysql_query("
			...
			(`person_id`, `name_surname`, `mail`, `address`, `phone`, `phone1`, `period`, `books`, `call_date`, `sum_price`, `callceenter_comment`, `operator_id`, `oder_send_date`, `task_id`)
			VALUES
			('$person_n', '$first_name', '$mail', '$addres', '$phone', '$phone1', '$send_date', '$row_name_prod', '$c_date', '$get_prod_row', '$result_comment', '$user', '', '$task_id')
				");
}
