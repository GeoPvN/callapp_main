...
//     <p>აღწერა: $task_comment</p><br><br><br>
//     <p>პერიოდი: $done_start_time - $done_end_time</p><br>
//     <p>ავტორი: $res[0]</p>
//     ";
							...
							<fieldset >
								...
								<table width="100%" class="dialog-form-table">
								   <tr>
									...
									<tr>
										<td style="width: 220px;"><select style="width: 220px;" id="task_type_id" class="idls object"><option value="10">გამავალი ზარი</option></select></td>
									    <td style="width: 220px;"><select style="width: 220px;" id="task_department_id" class="idls object"><option value="40">ქოლცენტრი</option></select></td>
...
function GetNote()
{
	$req = mysql_query("	SELECT 	`note` 
							FROM 	`phone`
							WHERE 	`actived` = 1 AND `note` != '' AND `import` = 1
							GROUP BY `note`");
...
function GetSorce()
{
	$req = mysql_query("	SELECT 	`sorce` 
							FROM 	`phone`
							WHERE 	`actived` = 1 AND `sorce` != '' AND `import` = 1
							GROUP BY `sorce`");
	$data .= '<option value="" selected="selected"></option>';
	while( $res = mysql_fetch_assoc($req)){
