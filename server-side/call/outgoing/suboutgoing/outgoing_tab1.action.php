...
switch ($action) {
    ...
    case 'all_products':
        ...
        $rResult = mysql_query("SELECT 	`production`.`id`,
                                        `production`.`name`,
                                        CONCAT('<input min=\"1\" onchange=\"changemypr(\'',`production`.`id`,'\')\" onclick=\"$(this).keydown(false)\" style=\"width: 40px;\" type=\"number\" value=\"1\" id=\"sub3_qu',`production`.`id`,'\">') as `qu`,
                                        CONCAT('<div id=\"sub3_rpr',`production`.`id`,'\">',`production`.`price`,'</div>'),
                                        CONCAT('<div id=\"sub3_pr',`production`.`id`,'\">',`production`.`price`,'</div>'),
                                        `production`.`description`,
                                        `production`.`comment`
                                FROM 	`production`
        ...
        while ( $aRow = mysql_fetch_array( $rResult ) )
        {
            ...
            for ( $i = 0 ; $i < $count ; $i++ )
            {
                ...
                $row[] = $aRow[$i];
                if($i == ($count - 1)){
                    $row[] ='<input  type="checkbox" id="all_p' . $aRow[0] . '" name="check_' . $aRow[0] . '" class="check_all_p" value="' . $aRow[0] . '" />
    					     <input  type="hidden" id="all_num'.$aRow[0].'"  class="all_num" value="1" />';
                }
            }
