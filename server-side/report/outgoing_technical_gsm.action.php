...
if($_REQUEST['act'] =='answear_dialog_table'){
	...
	$rResult = mysql_query("SELECT cdr.calldate,
								       ...
								       cdr.dst,
									   persons.`name`,
								       SEC_TO_TIME(cdr.billsec) as `time`,
								       CONCAT('<p onclick=play(', '\'', SUBSTRING(cdr.userfield, 7), '\'',  ')>მოსმენა</p>', '<a download=\"',cdr.src,'-',cdr.calldate,'.vaw\" href=\"http://109.234.117.182:8181/records/', SUBSTRING(cdr.userfield, 7), '\">ჩამოტვირთვა</a>')
						    FROM   cdr
	                        JOIN users ON users.ext = cdr.src
...
if($_REQUEST['act'] =='unanswear_dialog_table'){
    ...
    if($_REQUEST['act'] =='busy_dialog_table'){
    $data		= array('page' => array(
        'answear_dialog' => ''
    ));
    $count = 		$_REQUEST['count'];
...
if($_REQUEST['act'] =='unanswear_dialog'){
    ...
    if($_REQUEST['act'] =='busy_dialog'){

    $data['page']['answear_dialog'] = '

				
												                <table class="display" id="busy_table">
												                    <thead>
												                        <tr id="datatable_header">
												                            <th>ID</th>
												                            <th style="width: 100%;">თარიღი</th>
...
//------------------------------- ტექნიკური ინფორმაცია
	...
	$row_busy = mysql_fetch_assoc(mysql_query("	SELECT 	COUNT(*) as count,
                                                    	    cdr.src,
                                                    	    cdr.dst,
                                                    	    SUBSTRING(cdr.lastdata,5,7)
                                            	    FROM    cdr
	...
	$data['page']['technik_info'] = '
                    ...
                    <td>ზარი</td>
                    <td>'.($row_answer[count] + $row_abadon[count] + $row_busy[count]).'</td>
                    ...
                    <td id="BUSY_dialog" style="cursor: pointer; text-decoration: underline;">'.$row_busy[count].'</td>
                    <td>'.round(((($row_answer[count]) / ($row_answer[count] + $row_abadon[count] + $row_busy[count])) * 100),2).' %</td>
                    <td>'.round(((($row_abadon[count]) / ($row_answer[count] + $row_abadon[count] + $row_busy[count])) * 100),2).' %</td>
                    <td>'.round(((($row_busy[count]) / ($row_answer[count] + $row_abadon[count] + $row_busy[count])) * 100),2).' %</td>
